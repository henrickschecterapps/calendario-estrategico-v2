...
import { Loader2, Calendar as CalendarGridIcon, Users, Package, Activity } from "lucide-react";
  ...
  return (
    <div className="flex flex-col h-screen w-full bg-bg overflow-hidden text-text">
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* FILTROS LATERAL ESQUERDA CLÁSSICO */}
        <SidebarFilters />

        {/* FEED PRINCIPAL TIMELINE */}
        <main className="flex-1 flex flex-col min-w-0 bg-bg">
          {/* Header — compact single row */}
          <header className="flex-shrink-0 px-8 md:px-10 py-4 flex items-center justify-between gap-4">
            <div className="flex items-center gap-6">
              <div>
                <h1 className="text-xl font-black text-text tracking-tight leading-none">Calendário Estratégico</h1>
                <p className="text-[10px] font-bold text-muted uppercase tracking-widest mt-0.5">Gestão Integrada de Eventos</p>
              </div>
              <button 
                onClick={() => setShowPastEvents(!showPastEvents)}
                className="hidden sm:flex bg-surface border border-border shadow-sm px-4 py-1.5 rounded-full text-[9px] font-black text-muted uppercase tracking-widest hover:text-accent hover:border-accent/30 transition-all active:scale-95"
              >
            ...
            <div className="flex items-center gap-3">
              <button 
                ...
                className="bg-accent hover:bg-accent-hover text-white px-5 py-2 rounded-xl font-black text-xs shadow-lg shadow-accent/15 transition-all active:scale-[0.97] flex items-center gap-2"
              >
                <Activity className="w-3.5 h-3.5" />
                Novo Evento
              </button>
              
              <div className="hidden lg:flex items-center bg-surface/50 p-1 rounded-xl border border-border shadow-sm">
                {(["timeline", "month", "week", "list"] as const).map((view) => (
                   <button 
                    key={view}
                    onClick={() => setActiveView(view)}
                    className={`px-4 py-1.5 text-[9px] font-black uppercase tracking-widest rounded-lg transition-all ${activeView === view ? "bg-surface text-accent shadow-sm border border-border" : "text-muted hover:text-text"}`}
                   >
          ...
          <div className="flex-1 overflow-y-auto px-8 md:px-10 pb-8 relative flex flex-col h-full min-h-0 custom-scrollbar">
