...
import { Bell, LogOut, Download, Moon, Sun, Settings, X, Info, AlertTriangle, Calendar, Search } from "lucide-react";
...
export default function TopBar() {
  ...
  return (
    <header className="h-20 w-full bg-surface/90 backdrop-blur-xl sticky top-0 flex items-center justify-between px-10 flex-shrink-0 z-[60] border-b border-border transition-all duration-500">
      <div className="flex items-center gap-5">
        <div className="w-11 h-11 bg-text rounded-2xl flex items-center justify-center shadow-xl shadow-text/10 group cursor-pointer hover:rotate-12 transition-transform" onClick={() => router.push('/dashboard')}>
          <Calendar className="w-6 h-6 text-bg" />
        </div>
        <h1 onClick={() => router.push('/dashboard')} className="text-2xl font-black text-text tracking-tighter cursor-pointer group flex items-center gap-1">
          Tripla <span className="text-accent group-hover:text-accent-light transition-all duration-300">Eventos</span>
        </h1>
      </div>
            ...
            <>
              ...
              <div className="absolute right-0 mt-5 w-96 bg-surface border border-border rounded-[28px] shadow-2xl z-40 overflow-hidden animate-in fade-in zoom-in-95 slide-in-from-top-4 duration-300">
                ...
                <div className="p-5 bg-bg/50 text-center border-t border-border">
                   <button className="text-[10px] font-black text-muted uppercase tracking-[0.2em] hover:text-accent transition-colors">Visualizar todo o histórico</button>
                </div>
              </div>
            </>
          )}
