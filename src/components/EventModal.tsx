...
export default function EventModal() {
  ...
  const unifiedBrindes = useMemo(() => {
    ...
    if (selectedEvent?.brindes_alocados) {
      selectedEvent.brindes_alocados.forEach((b) => {
  ...
  // eslint-disable-next-line react-hooks/set-state-in-effect -- async data loading on prop change
  useEffect(() => {
  ...
  // eslint-disable-next-line react-hooks/set-state-in-effect -- resetting modal state when selected event changes
  useEffect(() => {
             ...
             return (
               <div className="grid grid-cols-3 gap-3">
                  {([
                    ...
                    { icon: <Package className="w-4 h-4"/>, label: 'Brindes', value: (selectedEvent.brindes_alocados?.length ?? 0) > 0 ? selectedEvent.brindes_alocados!.map((b) => `${b.qtd}x ${b.item}`).join(', ') : (selectedEvent.brinde ?? '') },
                    { icon: <Box className="w-4 h-4"/>, label: 'Materiais', value: selectedEvent.materiais ?? '' }
                  ] as Array<{ icon: React.ReactNode; label: string; value: string }>).map((box, i) => (
