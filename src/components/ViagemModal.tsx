...
import type { Viagem, ViagemFormData } from "@/types/collections";

interface ViagemModalProps {
  isOpen: boolean;
  onClose: () => void;
  itemToEdit?: Viagem | null;
  onSaved: () => void;
}
...
export default function ViagemModal({ isOpen, onClose, itemToEdit, onSaved }: ViagemModalProps) {
  ...
  useEffect(() => {
    if (itemToEdit) {
      setFormData({
        trecho: itemToEdit.trecho || '',
        passageiro: itemToEdit.passageiro || '',
        status: itemToEdit.status || 'Pendente',
        localizador: itemToEdit.localizador || '',
        valor: itemToEdit.valor || '',
        arquivos: itemToEdit.arquivos || []
      });
    } else {
      setFormData({
        trecho: '',
