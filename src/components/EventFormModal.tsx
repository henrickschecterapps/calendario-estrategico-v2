...
export default function EventFormModal({ isOpen, onClose, eventToEdit, onSaved }: EventFormModalProps = {}) {
  ...
  const [formData, setFormData] = useState<EventFormData>({
    ...
    clientes: [] as ClienteMember[],
    vips: [] as VipMember[],
    brindes_alocados: [] as BrindeAlocado[],
    publico: '',
    participantes: '',
    beneficios: '',
    obs: '',
  ...
  const [newBrinde, setNewBrinde] = useState({ id: '', item: '', qtd: 1, _collection: 'inventario' as InventoryCollection });
  const [inventoryItems, setInventoryItems] = useState<InventoryItem[]>([]);
  const [participantesDb, setParticipantesDb] = useState<Participante[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchInventory = async () => {
      try {
        ...
        snapInv.forEach(d => allItems.push({ ...d.data(), _collection: 'inventario' }));
        snapBri.forEach(d => allItems.push({ ...d.data(), _collection: 'brindes', tipo: 'brinde' }));
    ...
    const fetchParticipantes = async () => {
      try {
        ...
        setParticipantesDb(snap.docs.map(d => d.data()));
      } catch (err) {}
    };
    fetchParticipantes();
  }, [finalOpen]);

  // eslint-disable-next-line react-hooks/set-state-in-effect -- syncing form data from props is the standard pattern for modal forms
  useEffect(() => {
    if (finalOpen) {
      if (finalEvent) {
        setFormData({
          evento: finalEvent.evento || '',
          data_ini: finalEvent.data_ini || '',
          data_fim: finalEvent.data_fim || '',
          hora_ini: finalEvent.hora_ini || '',
          hora_fim: finalEvent.hora_fim || '',
          responsavel: finalEvent.responsavel || '',
          tipo: finalEvent.tipo || 'Comercial',
          status: finalEvent.status || 'Planejado',
          formato: finalEvent.formato || 'Presencial',
          cota: finalEvent.cota || '',
          localidade: finalEvent.localidade || '',
          links: finalEvent.links || '',
          organizadores: finalEvent.organizadores || [],
          vagas_staff: finalEvent.vagas_staff || 0,
          vagas_cliente: finalEvent.vagas_cliente || 0,
          vagas_vip: finalEvent.vagas_vip || 0,
          equipe: finalEvent.equipe || [],
          clientes: finalEvent.clientes || [],
          vips: finalEvent.vips || [],
          brindes_alocados: finalEvent.brindes_alocados || [],
          publico: finalEvent.publico || '',
          participantes: finalEvent.participantes || '',
          beneficios: finalEvent.beneficios || '',
          obs: finalEvent.obs || '',
          arquivos: finalEvent.arquivos || [],
          historico: finalEvent.historico || []
        });
      } else {
        setFormData({
          evento: '', data_ini: '', data_fim: '', hora_ini: '', hora_fim: '', responsavel: '', tipo: 'Comercial', status: 'Planejado', formato: 'Presencial', cota: '', localidade: '', links: '', organizadores: [], vagas_staff: 0, vagas_cliente: 0, vagas_vip: 0, equipe: [], clientes: [], vips: [], brindes_alocados: [], publico: '', participantes: '', beneficios: '', obs: '', arquivos: [], historico: []
        });
      }
    }
  ...
  const addEquipe = () => {
    ...
    setFormData((prev) => ({
      ...prev,
      equipe: [...prev.equipe, { ...newEquipeInfo }]
    }));
  ...
  const addCliente = () => {
    ...
    setFormData((prev) => ({
      ...prev,
      clientes: [...(prev.clientes || []), { ...newClienteInfo }]
    }));
  ...
  const addBrinde = async (e?: React.MouseEvent) => {
    ...
    const invItem = inventoryItems.find((i) => i.id === newBrinde.id);
    ...
    try {
      const db = getFirebaseDb();
      const col = invItem._collection || 'inventario';
      const currentDoc = await getDoc(doc(db, col, newBrinde.id));
      const currQtd = Number(currentDoc.data()?.quantidade) || 0;
      await updateDoc(doc(db, col, newBrinde.id), {
      ...
      setFormData((prev: any) => ({
        ...prev,
        brindes_alocados: [...(prev.brindes_alocados || []), { ...newBrinde, _collection: col, docId: Date.now().toString() }]
      }));
      setNewBrinde(prev => ({ ...prev, qtd: 1 }));
      showToast('Estoque abatido e item alocado!', 'success');
    } catch (err: any) {
  ...
  const removeBrinde = async (e: React.MouseEvent, b: any) => {
    ...
    try {
      const db = getFirebaseDb();
      if (b.id) {
         const col = b._collection || 'inventario';
         const currentDoc = await getDoc(doc(db, col, b.id));
         if (currentDoc.exists()) {
             const currQtd = Number(currentDoc.data()?.quantidade) || 0;
             await updateDoc(doc(db, col, b.id), {
                       ...
                       <div className="flex-1">
                         ...
                         <select 
                           ...
                           onChange={e => {
                             ...
                             if (selected) setNewBrinde({ id: selected.id, item: selected.nome, qtd: newBrinde.qtd, _collection: selected._collection || 'inventario' });
                           }} 
