...
import { EventHistoryEntry } from "@/types/collections";
import { genericConverter } from "./firebaseUtils";
 ...
 */
export async function fetchEventHistory(eventId: string): Promise<EventHistoryEntry[]> {
  const { getFirebaseDb } = await import("@/lib/firebase");
  ...
  try {
    const q = query(
      collection(db, "eventos", eventId, "history").withConverter(genericConverter<EventHistoryEntry>()),
